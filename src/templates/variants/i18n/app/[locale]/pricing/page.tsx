...
type PlanTranslation = { name: string; description: string; features: string[]; cta: string }
type CreditPackTranslation = { name: string; description: string }
...
export default async function PricingPage() {
  ...
  const plansTranslations = t.raw("plans_items") as PlanTranslation[]
  const creditPacksTranslations = t.raw("credit_packs_items") as CreditPackTranslation[]

  const localizedPlans = plans.map((p, i) => ({
    ...p,
    name: plansTranslations[i]?.name ?? p.name,
    description: plansTranslations[i]?.description ?? p.description,
    features: plansTranslations[i]?.features ?? p.features,
    cta: plansTranslations[i]?.cta ?? p.cta,
  }))

  const localizedPacks = creditPacks.map((p, i) => ({
    ...p,
    name: creditPacksTranslations[i]?.name ?? p.name,
    description: creditPacksTranslations[i]?.description ?? p.description,
  }))
  ...
  function formatPlanPrice(price: number, period: string | null) {
    if (price === -1) return { amount: t("price_custom"), period: null }
    if (price === 0) return { amount: t("price_free"), period: null }
    const amount = (price / 100).toFixed(2).replace(".00", "") + "€"
    return {
      amount,
      period: period === "month" ? t("price_month") : period === "year" ? t("price_year") : null,
    }
  }
                      ...
                      <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                        <div className="badge-beam">
                          ...
                          <div className="badge-beam-inner gap-1.5 px-3 py-1 text-xs">
                            ...
                            <span className="bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text text-transparent">{t("most_popular")}</span>
                          </div>
                        </div>
                      </div>
                    )}
              ...
              <div className="mb-12 text-center">
                <div className="badge-beam mb-4" data-gsap="badge">
                  ...
                  <div className="badge-beam-inner gap-1.5">
                    ...
                    <span className="bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text text-transparent">{t("credits_badge")}</span>
                  </div>
                </div>
                ...
                <p className="mt-3 text-muted-foreground" data-gsap="subtitle">{t("credits_subtitle")}</p>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-baseline gap-1.5">
                        ...
                        <span className="text-muted-foreground text-sm">{t("credits_label")}</span>
                      </div>
                      ...
                      <div className="mt-auto space-y-3">
                        ...
                        <Button
                          ...
                          asChild
                        >
                          <Link href="/register">{t("credits_buy")}</Link>
                        </Button>
                      </div>
                    </div>
                  )
